...
**Noise.** Clean, and signal-to-noise ratios of 0 dB and −5 dB. The harness sets the noise by its level L relative to the speech: noise amplitude is speech RMS times 10^(L/20), so L = 0 is noise at speech-equal power (0 dB) and L = +5 is noise 5 dB louder than the speech (−5 dB), drawn over the whole padded window. The record and the run identities keep L (`5db`), since it seeds the noise; the paper, the summary and the figures report the signal-to-noise ratio (`summary.snr`). Each clip's noise comes from a generator seeded by the clip's identity and the level, so a clip sounds the same in every arm, seed, tier and process. Seeds change which clips a readout is fitted on and an arm's own random draws, never the clips themselves. Front-end rows are computed once per input pathway and level and memory-mapped by every run.
...
| cochlea | the coil, the cochlea and the cochlea at the coil's average coupling, for the four phase coupling functions and the three kinds of natural frequencies; restoring strength 0.3, ceiling 1; 0 and −5 dB, both gains, three seeds (decision log) | 432 |
...
**2026-09-24 06:10 MDT. The trained baselines retrained.** The CNN and TCN failed to train in 17 of their 36 recognition runs with noise, and on the order task every S4D, CNN and TCN run with noise, and 2 GRU runs, stayed at the chance loss from the first epoch to the last. The cause was the recipe, not the task: the learned head read the pooled statistics unstandardized, and with noise their spread between clips was often a thousandth of their size or zero (half of the S4D's and CNN's order features did not vary between clips at all), so no gradient reached the networks; and the CNN and TCN ended in a rectified layer of 16 channels, which a poor draw could leave almost entirely inactive (a failed TCN seed had 7 of 16 output units ever active at initialization). The fix, applied to every trained baseline: the head now reads the statistics through a parameter-free standardization (batch normalization without learned scale or shift), as the ridge reads them standardized; the CNN's output layer is linear; and the TCN, which was a second two-layer convolution rather than a temporal convolutional network, is now one: two residual blocks of dilated causal convolutions (dilations 1, 2 and 4, 8, kernel 3, receptive field 31 frames, 1,972 parameters). The features the ridge reads are unchanged in kind. Rerun on the failed cases, every recognition run trained (the TCN's six at 0 and +5 dB went from 10.0 to 62.8% to 75.9 to 83.5%); on the noisy order task the S4D's and CNN's training loss now falls but their test accuracy stays at chance, so that result is now the architectures' and not the recipe's. Every trained-baseline run in Tier 1 and Tier B is rerun with the fixed recipe and replaces the earlier one; the earlier runs remain in the history (commit 978659c). Reason: a baseline that did not train measured the recipe, not the architecture.

**2026-09-24 06:10 MDT. A sweep beyond Tier 2's levels.** Tier 2 moved accuracy least through the restoring strength and the coupling ceiling, and the Stuart–Landau network held its accuracy from gain 1 to gain 2 while Kuramoto's fell. A new tier, `sweep`, runs every coupling function at the reference configuration at restoring strengths 0.5, 0.8 and 1.0 (up to the natural frequency, where an oscillator stops rotating) and coupling ceilings 1.5 and 2.0, at gains 1 and 2, and at gains 3, 4, 5, 6, 8, 10 and 12 at the reference settings; 0 and +5 dB, three seeds, the primary cell, paired with Tier 2's reference cells: 612 runs, on the Apple GPU. Gain stops at 12 because at 16 the drive, the natural frequency and the coupling together pass π radians a step, the integrator's bound. Reason: the author asked whether the trends continue past the levels run, and where Stuart–Landau's tolerance of gain ends.

**2026-09-24 06:10 MDT. A coil and a cochlea.** The paper described the cylinder and sphere as leaving the frequency axis open "as the cochlea does", but no geometry was a cochlea, and the helix is a pitch helix (a closed ring, one octave per turn), not a cochlear spiral. Two geometries are added (`harness.models.geometries.coil`): the coil, one open line from base (highest band) to apex (lowest band) with one octave per turn, so adjacent turns share kernel taps and the ends never meet; and the cochlea, the coil with the travelling wave's direction (taps toward the apex ×1.5, toward the base ×0.5) and the coil's curvature (the coupling into each site scaled from 1 at the apex to 0.25 at the base, a logarithmic spiral whose radius falls to a quarter). A new tier, `cochlea`, runs both for the four phase coupling functions and the three kinds of natural frequencies (tonotopic frequencies follow the coil's exponential map), at the reference restoring strength and ceiling, 0 and +5 dB, both gains, three seeds: 288 runs, paired with Tier 2's torus and helix cells. The Stuart–Landau core has no geometry support, so it is not run on them. Reason: the author asked for a proper cochlear geometry after the comparison with a cochlea-inspired resonator on AudioMNIST (Beoletto et al. 2026).

**2026-09-24 10:05 MDT. A cochlea at the coil's average coupling.** The cochlea read up to a point below the coil, but its curvature weights, capped at 1 at the apex so the ceiling stays a true bound, cut its average coupling to 0.54 of the coil's, and its drop matched that of halving the coupling ceiling in Tier 2, so the result could not separate the cochlea's mechanics from its weaker coupling. A third geometry, `cochlea-matched`, keeps the cochlea's direction and shape of weighting with the weights rescaled to average 1, the coil's average coupling; at its apex the coupling rises to about 1.85 times the ceiling, so for it the ceiling bounds the average rather than the strongest coupling. It joins the `cochlea` tier on the same terms (144 runs). Reason: the author asked for the control.

**2026-09-24 13:16 MDT. Noise levels reported as signal-to-noise ratios.** The noise levels were labelled by the harness's convention, the level of the noise relative to the speech, so "+5 dB" meant noise 5 dB louder than the speech, a signal-to-noise ratio of −5 dB. The studies the paper compares with, and most of the field, label by signal-to-noise ratio, where +5 dB means the reverse, so the paper, the summary and the figures now report the signal-to-noise ratio: 0 dB and −5 dB. Nothing is rerun and no result changes: the record and the run identities keep the harness's level (`5db`), which seeds each clip's noise, and `summary.snr` converts it for display. "+5 dB" in the entries above is −5 dB in the paper. Reason: the author found the label confusing, and it read the wrong way round beside Beoletto et al. 2026 and Young et al. 2026.
