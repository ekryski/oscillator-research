...
In brief: most of the accuracy comes from the input and the readout. At a common readout width the untrained Kuramoto network reads about as well as its own input on recognition, and carries the order of events by integrating its input, as a non-oscillating leaky-integrator bank does. A free oscillator amplitude (Stuart–Landau) is the one design choice that moved accuracy by more than about a point, and it also lets the network tolerate a stronger input; the lattice geometry, a cochlear coil included, moved it by less than a point. Small trained networks, trained with a recipe that lets them train, read above every untrained reservoir with noise but the Stuart–Landau network.
...
Widening the readout from 192 to 4,096 features raised the Kuramoto network by 2.1 to 6.3 points and the leaky-integrator banks by up to 6.2 points; the spectrogram-only baseline and four trained baselines are 192 wide and unchanged. From 2,048 to 24,000 training clips, at width 192, the baseline gained 3.5 to 4.1 points, the Kuramoto network 2.0, and the trained baselines 7.1 to 12.4 (Appendix H). With 24,000 clips and width 4,096, the network read 85.9% at 0 dB and the state-matched bank 84.8%, against 82.1% for the baseline and 86.7% to 90.6% for the trained baselines. Read through a projection drawn per seed rather than one fixed draw, Tier 1's reservoirs moved by −1.29 to +1.22 points, 35 of 48 paired intervals containing zero, and every paired comparison between Tier 1's arms kept its sign.
...
**Readout, data and training.** More training data raised the trained baselines three to six times as much as the reservoirs, since in a reservoir only the readout learns; with noise and 2,048 clips every trained baseline already reads above the Kuramoto network, and only the Stuart–Landau network reaches their range. Published accuracies on noisy AudioMNIST are not comparable with these: both studies we found share speakers between training and test ([Beoletto et al. 2026](https://doi.org/10.1002/aisy.202500526); [Young et al. 2026](https://doi.org/10.1186/s13636-026-00457-2)), which lets a classifier learn the test speakers' voices and likely raises its accuracy. Training the oscillator network itself is the step these results leave open. A trained coupled-oscillator network reaches 89% on AudioMNIST from the raw waveform where a plain recurrent network fails ([Chandravadia & Imam 2026](https://doi.org/10.1016/j.patter.2026.101563)), trained end to end on a random split; running such a network through the controls used here, on held-out speakers, would show what its oscillators contribute.
...
The code and the complete run record (every run's specification, results and per-clip correctness) are in the anonymized supplementary material and will be released with the paper; AudioMNIST is public ([Becker et al. 2024](https://doi.org/10.1016/j.jfranklin.2023.11.038)). From the record alone, one command regenerates every table and figure in this paper, and from the audio, the harness reruns any tier, resuming without repeating a finished run. Every run records its code version, hardware and library versions. Tiers 1 and 2 ran on the CPU of an Apple M1 Max, where runs with the same seed are bit-identical. The later tiers simulated the reservoirs on its GPU, which reproduced 1,262 of 1,296 of Tier 1's cells exactly and all but one within two test clips (Section 3.5); the trained baselines train on the CPU on every device. We welcome reproductions.
...
The tiers of the study and their run counts.
...
| B, Becker | Where do we sit against published numbers? | the Tier 1 arms | the 5 speaker folds of [Becker et al. 2024](https://doi.org/10.1016/j.jfranklin.2023.11.038); input gain 1 and 2 | clean audio; 18,000 training, 6,000 validation and 6,000 test clips; seed 0 | 70 |
...
| sweep | How far do restoring strength, coupling ceiling and gain move accuracy beyond Tier 2's levels? | coupled oscillator network, all six coupling functions | restoring strength 0.5, 0.8, 1.0 and coupling ceiling 1.5, 2.0 at input gain 1 and 2; input gain 3, 4, 5, 6, 8, 10, 12; noise 0 and +5 dB; seeds 0–2 | torus, random natural frequencies; 2,048 training clips | 612 |
...
Table: The eight lattice geometries. The first six run in Tier 2; the coil and the cochlea run in a tier of their own (Appendix B).
...
It ran in eight tiers of 6,275 runs (Appendix B). Every run is one arm, one of the systems compared, at one noise level, input gain and random seed; every oscillator network is 4 channels of a 16 × 16 lattice. Each arm's signals are summarized, brought to a common width and classified by the same linear readout (Section 3.3), each condition is run at three seeds, and every accuracy is measured on speakers held out from training (Section 3.2). The runs used an Apple M1 Max (10 cores, 64 GB). Tiers 1 and 2 ran on its CPU; from Tier B on, the reservoirs were simulated on its GPU, while the readout and the trained baselines stayed on the CPU. The two devices round differently: rerun on the GPU, Tier 1's reservoir runs reproduced 1,262 of their 1,296 recorded cells exactly and all but one within two test clips, and in the last the readout chose a different penalty and read 0.63 points lower. Appendix A defines every term.
...
Every coupled and uncoupled oscillator network in this section, and in Sections 4.2, 4.6 and 4.7, is the Tier 1 configuration: Kuramoto coupling on the torus, with random natural frequencies, restoring strength 0.3 and coupling ceiling 1; the other coupling functions, among them Stuart–Landau, are compared in Section 4.3. The table gives recognition accuracy at the primary cell for the spectrogram-only baseline, the three reservoirs at input gain 1, and the GRU, the trained baseline with the highest accuracy with noise; the figure in Section 4.1 adds each reservoir at gain 2.
...
Table: Each design level minus its reference level, over matched configurations: difference in recognition accuracy (points), mean ± standard deviation over three seeds; ω, natural frequencies; λ, restoring strength; ceiling, coupling ceiling. The coil and cochlea rows pair the four phase coupling functions and three kinds of natural frequencies at the reference restoring strength and ceiling. The 95% paired intervals for Tier 2's rows are in the figure of Section 4.3.


### H.4 Sensitivity of the canonical physics values

The restoring strength and the coupling ceiling are crossed with every other factor in Tier 2, so each difference is paired over 156 configurations. A restoring strength of 0.1 read 0.13 to 0.23 points below 0.3, and a coupling ceiling of 0.5 read 0.39 to 0.68 points below 1, in all four conditions (the last rows of the table in Appendix H.3).

Adding each oscillator's rotation rates to the read raised accuracy by 0.13 to 0.17 points at gain 1 and by 0.47 to 0.49 points at gain 2, averaged over all 312 configurations. Per-clip correctness is kept only for the primary read in Tier 2, so this difference has no bootstrap interval.

A sweep tier extends the restoring strength, the coupling ceiling and the gain beyond Tier 2's levels, for every coupling function at the reference configuration, paired with Tier 2's reference cells on the same clips and seeds (the sweep's reservoirs ran on the Apple GPU and Tier 2's on its CPU, which agree within two test clips in all but one of 1,296 recorded cells; Section 3.5). Restoring strengths of 0.5, 0.8 and 1.0 moved the phase-oscillator networks by −1.2 to +2.1 points against 0.3, with Kuramoto 0.2 to 0.8 points above; they lowered the Stuart–Landau network in every condition, by 0.4 to 2.8 points, more as the restoring strength grew. Coupling ceilings of 1.5 and 2 moved every coupling function by −2.4 to +1.7 points against 1, with no direction common to them.
...
Read through a projection drawn for each run seed instead of the one fixed draw, Tier 1's reservoirs moved by −1.29 to +0.88 points on recognition and −0.63 to +1.22 points on order (seeded minus fixed, 48 paired comparisons, 35 of whose 95% intervals contain zero). Under the seeded projection the coupled network read 2.27, 0.40 and 0.69 points below the whole-clip baseline on recognition at gain 1 (clean, 0 dB, +5 dB), against 2.53, 0.23 and 0.88 under the fixed one; 0.58 to 1.46 points above the uncoupled network (0.43 to 1.08 fixed); and 4.0 to 4.8 points above the state-matched bank with noise (4.0 to 4.2 fixed).

<!-- TODO: Coherence: every network run from Tier B on records the order parameter R, phase locking to each band's drive, the share of oscillators locked and the mean amplitude; the projection tier reruns Tier 1's networks, so they have them too, while Tier 2's runs do not. Decide whether the paper reports them or leaves them to paper 03. -->
