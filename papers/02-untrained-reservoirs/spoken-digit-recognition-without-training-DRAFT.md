...
The confirmatory study was registered before its first run and is organized in seven tiers. Tier 4 was widened after the registration was frozen and before any of its runs, from 1, 4 and 16 channels of a 16 × 16 lattice at input gain 2 to the design below. Every run is one arm at one noise level, input gain and seed, read at several readout widths and training sizes; the run counts below are the registered totals. Appendix A defines every term.
...
| **Row** | The oscillators of one channel that a single mel band drives, 16 on the 16 × 16 lattice. The only grouping built into the network. |
| **Band mapping** | How a front end's mel bands drive a lattice's rows. On the 16 × 16 lattice, one band per row. A lattice of another size is driven either by its own number of mel bands, one per row, or by the 16 bands mapped onto its rows: each band drives two adjacent rows of a 32 × 32 lattice, and each row of an 8 × 8 lattice is driven by the mean of two adjacent bands. Nothing in the mapping is fitted. |
