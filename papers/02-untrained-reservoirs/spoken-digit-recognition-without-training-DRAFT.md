...
**Readout, data and training.** More training data raised the trained baselines three to six times as much as the reservoirs, since in a reservoir only the readout learns; with noise and 2,048 clips every trained baseline already reads above the Kuramoto network, and only the Stuart–Landau network reaches their range. Training the oscillator network itself is the step these results leave open. A trained coupled-oscillator network reaches 89% on AudioMNIST from the raw waveform where a plain recurrent network fails ([Chandravadia & Imam 2026](https://doi.org/10.1016/j.patter.2026.101563)), trained end to end on a random split; running such a network through the controls used here, on held-out speakers, would show how much of that accuracy its oscillators contribute, and the untrained results here are the baselines to compare it against.

**Limitations.** Three seeds give a spread but not a precise estimate of it. The readout is one structure, a ridge on three statistics per signal. The trained baselines are matched at about 2,000 parameters and trained with one recipe, not tuned per architecture; a first version of that recipe fed the learned head unstandardized statistics, which with noise barely varied between clips, and left many CNN, TCN and S4D runs untrained, while the first TCN was a two-layer convolution. Rerun with the recipe reported here, they gained up to 66 points in noise, so a trained baseline's accuracy in a comparison like this can depend as much on its recipe as on its architecture. The later tiers ran on a GPU that rounds differently from the CPU, and a reproduction on a CUDA GPU is planned.

## 6 Conclusion

An untrained coupled oscillator network, read by a linear readout at a common width, reads about as well as its own input on noisy speaker-disjoint spoken-digit recognition, and carries the order of events by integrating its input, as a leaky-integrator bank does; coupling adds to both. A free oscillator amplitude is the one design choice that moved accuracy by more than about a point: it put the network above its input in every noisy condition and let it tolerate a much stronger input. The lattice geometry, a cochlear coil included, and the phase coupling function moved it by less than a point. Small trained networks read above every untrained reservoir with noise but the Stuart–Landau network, and these results give studies of trained oscillator networks a baseline to compare against.
