...
Coupled-oscillator networks are returning as a machine-learning substrate, often on a premise that oscillator physics turns signals into useful structure for more efficient machine learning models. That premise is rarely tested with comprehensive ablations and controls. We test it on speech classification of digits from the audioMNIST dataset by comparing trained networks of identical size and a non-oscillating field as controls to a small frozen (untrained) oscillator field combined with a closed-form linear probe. We do so across a pre-registered sweep of coupling laws, lattice geometries and audio input signal methods to give a comprehensive set of ablations that isolate which factors impact how well an untrained oscillator field performs on spoken digit classification. The outcome of our research is the measured impact that dynamics, coupling, field geometry, input signal and structure have on untrained baselines from which to inform future research into training oscillator field dynamics.
...
([Torrejon 2017](https://doi.org/10.1038/nature23011)), a figure the same group later qualified, showing that the cochlear front end alone reaches 95.8% on that task and that the high recognition level is "mainly due to the non-linear character of these frequency decomposition methods" rather than to the reservoir ([Abreu Araujo et al. 2020](https://doi.org/10.1038/s41598-019-56991-x)), which is the floor comparison this paper adopts; and a coRNN trained on raw 8 kHz waveform, 89 ± 1% on AudioMNIST under a random 90/10 split of the recordings, which is not speaker-disjoint ([Chandravadia & Imam 2026](https://doi.org/10.1016/j.patter.2026.101563)).
...
([Torrejon 2017](https://doi.org/10.1038/nature23011)); reservoir computing supplies the frame and its cautions ([Jaeger 2001](https://www.ai.rug.nl/minds/uploads/EchoStatesTechRep.pdf); [Maass 2002](https://doi.org/10.1162/089976602760407955); [Lukoševičius 2012](https://doi.org/10.1007/978-3-642-35289-8_36); [Tanaka 2019](https://doi.org/10.1016/j.neunet.2019.03.005)). Untrained random features with a linear classifier are an established strong family in time-series classification: Hydra draws large collections of random convolutional kernels and classifies with a simple linear model, and has been evaluated on AudioMNIST ([Saadatmand et al. 2026](https://arxiv.org/abs/2605.27406)), so an untrained arm with a linear readout doing well on this corpus is not by itself evidence for any particular dynamics. Oscillator-inspired trained networks (coRNN
