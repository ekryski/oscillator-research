...
Coupled-oscillator networks are returning as a machine-learning substrate, often on a reservoir premise: that oscillator physics turns signals into useful structure before any training. That premise is rarely tested under controls. We test it on speech. We freeze every parameter of a small oscillator field, read it with a closed-form linear probe, and compare it with trained networks of identical size and with a floor that bypasses the dynamics, across a pre-registered sweep of coupling laws, lattice geometries and input pathways. The premise splits in two. For recognizing spoken digits, frozen fields outperform every trained baseline of matched size, yet sit only a few points above what their input already offers a linear readout, and almost nothing in the field's design moves that margin: not its geometry, not its coupling law, not even frequency tuning that locks the oscillators to the sound. For memory it holds emphatically: on a task that order-blind readouts provably cannot solve, the same frozen fields recover temporal order with 0.97 to 1.00 accuracy, matching a recurrent network trained under the same readout, and a severed-coupling control traces most of that memory to individual oscillators rather than their interaction. What an untrained field is fed, and what it is asked, matter far more than how it is built. The results bound what oscillator physics gives for free and mark where training should matter next: the amplitude channel, the one design element that helped on both tasks.
