...
| folder | contents |
|---|---|
| `documents/` | the methods and results in full, as `methods-in-full.md` (with each experiment's design) and `results-in-full.md`: the detail the paper summarizes |
| `results/` | the complete record, 6,353 runs in the paper's eight experiments (`documents/methods-in-full.md` lists them), one file per experiment and task, with `summary.json` and `summary.md`; its [README](results/README.md) says what each file holds |
