...
def test_frames_outside_the_span_cannot_touch_the_read():
    ...
    # a read that pooled over both would hand one arm frames the others never see
    x = trajectory()
...
def test_the_seeded_projection_is_a_draw_of_its_own_per_seed_and_leaves_the_fixed_one_alone():
    ...
    assert torch.equal(ft.projection_matrix(500, 64), fixed)              # the fixed draw, unchanged
