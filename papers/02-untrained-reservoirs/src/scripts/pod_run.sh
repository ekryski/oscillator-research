...
# builds the bank and the row caches, runs the test suite, then the
# experiments; the summary reports the zero-input leak check. Every step is resume-safe, so re-running it after an
...
AUDIOMNIST="${1:?usage: pod_run.sh <path to the AudioMNIST checkout> [experiment ...]}"
shift || true
if [ "$#" -gt 0 ]; then EXPERIMENTS=("$@"); else EXPERIMENTS=(leak-check controls design becker-folds quadrature projection sweep cochlea); fi
...
# a controls network run holds about 8 GB of features, a design run about 2 GB; the
...
for experiment in "${EXPERIMENTS[@]}"; do
    case "$experiment" in
        leak-check|controls|becker-folds) W="$WIDE"; T=4 ;;
        *) W="$NARROW"; T=2 ;;
    esac
    uv run python -m harness.experiment.plan run "$experiment" --workers "$W" --threads "$T" --device "$DEVICE"
done
