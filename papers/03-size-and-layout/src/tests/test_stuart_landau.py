...
def test_fixedamp_sl_converges_to_phase_core_as_dt_shrinks():
    ...
    def gap(dt, substeps):
        ...
        torch.manual_seed(7)
        phase = OscillatorField(channels=2, grid=16,
                                 dt=dt, substeps=substeps)
        torch.manual_seed(7)
        sl = OscillatorField(channels=2, grid=16,
...
def test_sl_trainables_are_physics_only():
    torch.manual_seed(0)
    m = OscillatorField(channels=2, grid=8, core="sl")
    ...
    torch.manual_seed(0)
    b = OscillatorField(channels=2, grid=8, core="sl-fixedamp")
