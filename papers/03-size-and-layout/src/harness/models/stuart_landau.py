...
singularity, amplitude implicit for unconditional radial stability):
  ...
  1. explicit coupling + pinning + drive:
      ...
      *different* physics — a θ-dependent Adler-type coupling — recorded as a
      ...
      difference is float32 noise; phase difference scales as dt²).)
...
purely radial) — the phase core is this core's equal-amplitude slice, so the
fixed-amplitude core (renormalized every substep) reproduces Kuramoto up to
O(dt²) projection error. The signals are cat(ℑz, ℜz), in the phase core's
[sin θ, cos θ] order, so the read is shared unchanged.

The coupling ceiling is met as in the phase core (`harness.models.phase.ceiling_factor`):
exactly, up or down, in paper 03; paper 02's "cap" only scaled down. Torus only.
"""
...
class SLCore(nn.Module):
    ...
    def __init__(self, channels: int = 4, grid: int = 16, substeps: int = 1,
                 ...
                 seed: int = 42, kernel_scaling: str = "exact"):
        super().__init__()
    ...
    def peaks(self) -> torch.Tensor:
        """[C]: each channel's spectral peak before scaling."""
        return torch.fft.rfft2(self.kernel).abs().flatten(1).amax(dim=1)
