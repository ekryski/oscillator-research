...
def _cells(tier: str, rec: dict, run_id: str, source: str) -> list[Cell]:
    ...
    return [Cell(tier, s["drive"], s["noise_db"], s["gain"], s["seed"], s["arm"], label, c["read"], c["width"],
                 c["n_train"], c["acc"], c.get("correct"), rec["n_test"], source) for c in rec["cells"]]
...
def accuracies(cells: list[Cell]) -> list[dict]:
    ...
    for c in cells:
        key = (c.tier, c.drive, c.label, c.read, c.noise, c.gain, c.width, c.n_train)
        groups[key][f"seed{c.seed}"] = c.acc
        sources[key].add(c.source)
    fields = ("tier", "drive", "arm", "read", "noise", "gain", "width", "n_train")
    out = []
...
def compare(cells: list[Cell], name: str, a, b, *, per_channel: bool = True, extra: dict | None = None) -> list[dict]:
    ...
    channel count if `per_channel`), noise, gain, width, training size and seed. A cell without gain
    (the spectrogram-only baseline, a trained baseline) matches the other at every gain."""
    ...
    def key(c: Cell) -> tuple:
        return (c.drive, c.grid, c.bands, c.channels if per_channel else 0, c.noise,
                c.gain if b_gain else None, c.width, c.n_train, c.seed)
    idx = {key(c): c for c in bs}
    groups = defaultdict(list)
    for c in cells:
        other = idx.get(key(c)) if a(c) else None
        if other is not None:
            groups[(c.drive, c.grid, c.bands, c.channels, c.noise, c.gain, c.width, c.n_train)].append((c, other))
    return [{"comparison": name, **(extra or {}), "pathway": terms.PATHWAYS[k[0]], "grid": k[1], "bands": k[2],
             "channels": k[3], "noise": k[4], "gain": k[5], "width": k[6], "n_train": k[7], **paired(v)}
            for k, v in sorted(groups.items(), key=str)]
...
def report(s: dict, done: dict[str, tuple[int, int, int]]) -> str:
    ...
    lines = ["# Paper 03: results", "",
             ...
             f"{PRIMARY_SIZE:,} training clips, the four-window read.", "",
    ...
    for noise in plan.NOISES:
        for gain in plan.GAINS:
            lines += [f"## Size, {_noise(noise)}, gain = {gain:g}: coupled oscillator network accuracy", ""]
            ...
            for name in ("uncoupled oscillator network", "state-matched leaky-integrator bank",
                ...
                lines += _grid_table([r for r in cmp if prim(r) and r["comparison"] == title
                                      and r["noise"] == noise and r["gain"] == gain], _diff) + [""]
    return "\n".join(lines) + "\n"
