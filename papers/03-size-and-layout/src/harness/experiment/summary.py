...
from harness.experiment.readout import projection_of, unpack
...
def chance() -> dict:
    ...
    digit-sequence task's (harness.experiment.protocol.sequence_chance) at each length."""
...
def snr(n) -> str:
    """A noise level as reported: its signal-to-noise ratio. The record keeps the harness's level of the
    noise relative to the speech (protocol.add_noise), so a 5 would be an SNR of -5 dB; paper 03 runs 0 dB."""
    if n is None:
        return "clean"
...
SEEDS_TEXT = "mean ± standard deviation over seeds 0 to 2"
#: how a paired difference is reported, as every note below says
DIFF_TEXT = (f"paired on the same test clips and seeds: {SEEDS_TEXT}, with the 95% interval from resampling test "
             "clips in brackets, in points")
FIT_TEXT = (f"An arm with more than {PRIMARY_WIDTH} features is projected down to {PRIMARY_WIDTH}; one with fewer is "
            "read as it is.")


def _cell_note(task: str, length: int = 0) -> str:
    """What a table's cells are: the readout width, the training set, the read and the test set."""
    if task == "recognition":
        read, test = "the four-window read (frames 16 to 61)", "the 6,000 clips of speakers 49 to 60"
        train = f"{PRIMARY_SIZE:,} training clips"
    elif task == "order":
        read = f"the whole-span read (frames 16 to {pr.joined_frames(2)})"
        test = "2,048 sequences per digit pair from speakers 49 to 60, the five pairs' test clips taken together"
        train = f"{pr.ORDER_TRAIN:,} training sequences per pair"
    else:
        read = f"the whole-span read (frames 16 to {pr.joined_frames(length)})"
        test = f"{pr.SEQUENCE_TEST:,} sequences from speakers 49 to 60, each position named by a readout of its own"
        train = f"{pr.SEQUENCE_TRAIN:,} training sequences"
    return (f"Readout width {PRIMARY_WIDTH} and {train}, {read}; tested on {test}; {SEEDS_TEXT}, in percent. "
            f"Rows are lattices and band mappings, columns channel counts. {FIT_TEXT}")


def _task_section(acc, cmp, projection: str, experiment: str, task: str, title: str, note: str, pick=lambda r: True,
                  controls: tuple[str, ...] = (), length: int = 0) -> list[str]:
    """The coupled network's accuracy by lattice and channel count, and its comparisons, each table with a
    note above it saying what its cells are."""
    read = am.PRIMARY_READ[task]

    def prim(r):
        return (r["width"] == PRIMARY_WIDTH and r["n_train"] == PRIMARY_SIZE and r["projection"] == projection
                and r["task"] == task and pick(r))
    lines = [f"## {title}: coupled oscillator network accuracy ({projection} projection)", "",
             f"{_cell_note(task, length)} {note}", ""]
    ...
    for name in controls:
        lines += [f"### {name} ({title[0].lower() + title[1:]}, {projection} projection)", "",
                  f"The coupled network's cells of the table above minus the other arm's at the same size, "
                  f"{DIFF_TEXT}.", ""]
...
def report(s: dict, done: dict[str, tuple[int, int, int]]) -> str:
    ...
    lines = ["# Paper 03: results", "",
             ...
             "interval from resampling test clips in brackets, in points. Noise levels are signal-to-noise "
             f"ratios. Primary cell: width {PRIMARY_WIDTH}, {PRIMARY_SIZE:,} training clips, {snr(noise)} SNR, input "
             f"gain {gain:g}, the task's primary read (four windows for recognition, the whole span for the memory "
             "tasks). `README.md` says what each record file holds.", "",
             "What the projection does: every arm's standardized summary statistics are multiplied by a random "
             f"Gaussian matrix that maps them to the common readout width, {PRIMARY_WIDTH}, so a network of 262,144 "
             f"oscillators and the input's {PRIMARY_WIDTH} features are read by readouts of the same size. Every table "
             "is given under both projections: the fixed one, paper 02's matrix, one draw shared by every run and "
             "seed, and the seeded one, drawn afresh from each run's seed, whose spread over seeds includes the "
             "draw's own variability.", "",
    ...
    for projection in PROJECTIONS:
        ...
        for length in pr.SEQUENCE_LENGTHS:
            ...
            lines += _task_section(
                ...
                controls=tuple(common), length=length)
            lines += _task_section(
                ...
                pick=lambda r, length=length: r["length"] == length and r["position"] == "all", length=length)
        for what in ("one band per row minus 16 bands mapped onto the rows", "the longer window minus paper 02's"):
            ...
            lines += [f"## Recognition, {name} ({projection} projection)", "",
                      f"{_cell_note('recognition')} Each lattice's network minus the same network under the other "
                      f"{'band mapping' if 'bands' in what else 'analysis window'}, {DIFF_TEXT}.", ""]
        ...
        for a, b in COCHLEA_PAIRS:
            ...
            lines += [f"## Recognition, {name}, over the four phase coupling functions ({projection} projection)", "",
                      f"{_cell_note('recognition')} Matched on coupling function at each size, {DIFF_TEXT}.", ""]
            lines += _grid_table([r for r in cmp if r["comparison"] == name and r["projection"] == projection
                                  and r["width"] == PRIMARY_WIDTH and r["n_train"] == PRIMARY_SIZE], _diff) + [""]
    lines += ["## Synchronization and locking", "",
              ...
              f"exceeds {am.PLV_LOCK_THRESH:g}. Recognition, the spectrogram pathway, {snr(plan.NOISES[0])} SNR and "
              f"input gain {plan.GAINS[0]:g}, over the 6,000 test clips of speakers 49 to 60 and the read's frames.",
              ""]
