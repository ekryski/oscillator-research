...
channels (drawn with 16 rows and four channels). Writes
...
def spectrogram(ax, x, y, w, h, label=True):
    ...
    if label:
        ax.text(x + w / 2, y - 0.25, "mel spectrogram:\nG band energies, or\n16 mapped onto G rows", ha="center",
                va="top", fontsize=8)


def lattice(ax, x, y, s, name):
    """One channel: a G x G lattice (drawn with 16 rows) whose row r is driven by band r, shown by its colour."""
    cell = s / BANDS
    for r in range(BANDS):
...
def channels_panel(ax):
    ...
    names = ["channel 1: a G × G lattice", "channel 2", "channel 3", "channel C"]
    for name, y in zip(names, ys, strict=True):
        lattice(ax, x0, y, s, name)
