...
uv run python -m harness.confirm.plan estimate   # runs, CPU-hours and memory per tier and lattice
```
...
Forward passes are bit-identical on every geometry (`tests/test_geometries.py`), and every number comes from a forward pass of an untrained arm or a trained baseline's features. The streamed read's sums run in a different order from the in-memory read's, so the two agree to float32 rounding rather than bit for bit; an arm is read one way or the other by its size alone, the same way on every run.
