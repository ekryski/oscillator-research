...
| module | what it owns |
|---|---|
| `confirm/plan.py` | the tiers, the cells taken from paper 02 (`paper02_group`, `paper02_run`), the row caches (`cache_jobs`), the cost model (`seconds`, `memory_gb`), and the parallel driver |
| `confirm/benchmark.py` | `plan benchmark`: times one batch of every lattice and channel count on a device and extrapolates every run and tier |
...
| `confirm/protocol.py` | the bank, Protocol A, per-clip noise, the order and digit-sequence sets and clips, the front ends, the band mapping (`to_rows`), row caches |
| `confirm/summary.py` | every accuracy, instrument and paired difference with its spread, paper 02's cells folded in, the order task's pairs pooled and the sequence's positions taken together: `summary.json`, `summary.md` |
...
```bash
uv run python -m harness.confirm.plan benchmark --device cuda --out ../results/benchmark/cuda.json   # price the tiers on this GPU
uv run python -m harness.confirm.plan prepare                         # row caches: every pathway, band count, window, order and sequence set
...
uv run python -m harness.confirm.plan run size trained sequence --grids 8 16 32 --device mps --workers 2
uv run python -m harness.confirm.plan run size trained sequence --grids 64 128 --device auto --workers 2
