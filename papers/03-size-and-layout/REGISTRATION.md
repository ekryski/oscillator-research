...
9. Whether to measure the projection's own variability: each run could also read its primary cell under a few other draws of the matrix, at the cost of the readout alone, since the features are already in hand. The projection is fixed across seeds (paper 02's choice, which the reused cells depend on), so without this the reported spread omits it, and every comparison across the 4,096-state boundary also crosses from paper 02's draw to the streamed read's.
