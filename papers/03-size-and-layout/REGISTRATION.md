...
**Only the CPU is bit-identical to paper 02.** A GPU's floating-point results are close to the CPU's but not identical: its FFTs, matrix products and reductions sum in other orders, and over 61 steps the phases drift apart at the level of rounding. The tests hold every model, front end and trained baseline on MPS to the CPU (trajectories within 2 · 10⁻³, whole runs within a few test clips on the synthetic bank). Paper 02's cells are therefore reused only as recorded on its CPUs, the reuse gate runs on the CPU, and any paper 02 cell that paper 03 has to run itself (section 8) runs on the CPU whatever `--device` says. Comparisons within paper 03 pair runs of one device where the record allows, and a run's device is part of its record.
