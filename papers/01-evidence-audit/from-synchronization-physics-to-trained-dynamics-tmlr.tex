...
current landscape of ONNs. We organise thirteen published systems on two
questions: whether gradients reach the oscillator dynamics and whether a
...
out how coupled oscillator populations self-organise.
...
self-emerged oscillatory timing is the brain's fundamental organiser of
...
\item
  ...
  stability from an implicit discretisation and reaches roughly
...
works at a short gradient horizon (Section 2.3).
...
with quantitative approximation and generalisation bounds
...
available forms silently: additive drive (coRNN, LinOSS), phase-coupled
conditioning (AKOrN's projected conditioning, Un-0's conditioning
oscillators sin-coupled into the field), input written into natural
frequencies (WONN), and input-gated coupling strength (KomplexNet). No
paper in this set contrasts two injection forms under controls or
discusses the choice as a choice.
...
established decomposition \citep{sajjadi2018, kynkaanniemi2019}, and
sparse systems tolerate higher learning rates, with dense failures
blamed on collapse into global lock. All of this is blog-published, one
system, one modality. The physics is consistent with it: chimera
coexistence is generic for nonlocal finite-range coupling, modular
fabrics natively produce chimera-like states and metastable timescale
hierarchies \citep{hizanidis2016, litwin-kumar2012}, sparse recurrent
fabrics classify at high capacity through resonant amplification
...
by construction. Theory tracks the same line, with generalisation bounds
...
\citep{hayes2023, turian2020}, known since Rife and Boorstyn in 1974,
observed as spectral-gap gradient suppression in quantum circuits
...
\citep{rusch2025} obtains stability from implicit discretisation,
...
simulated & \textbf{blogs + MIT code \citep{mit-code}; not peer
reviewed} \\
\cmidrule{1-6}WONN \citep{wonn} & Winfree & whole model & dynamics;
input written as frequencies & simulated & preprint \\
\cmidrule{1-6}AKOrN \citep{akorn} & D-dimensional Kuramoto & layer in a
